...
public class SideBar extends View {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        //导航栏居中显示，上下各有80dp的边距
        mHeight = (int) (h - MyUtils.dp2px(mContext, TOTAL_MARGIN));
        mWidth = w;
    ...
    protected void onDraw(Canvas canvas) {
        for (int i = 0; i < indexStr.length(); i++) {
            ...
            canvas.drawText(textTag, xPos, singleHeight * (i + 1) + MyUtils.dp2px(mContext, TOP_MARGIN), mPaint);
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                int position = (int) ((event.getY() - getTop() - MyUtils.dp2px(mContext, 80)) / mHeight * indexStr.toCharArray().length);
